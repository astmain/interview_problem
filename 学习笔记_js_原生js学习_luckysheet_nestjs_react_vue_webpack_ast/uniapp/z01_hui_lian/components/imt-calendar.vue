<template>
	<view class="imt-calendar">
		<view class="calendar-month-wrapper">
			<view class="calendar-icon icon-zuojiantou" :style="current == 0 || {color: color}" @click="current == 0 || current--">&#xe6ab;</view>
			<view class="calendar-month">{{currentTime}}</view>
			<view class="calendar-icon calendar-icon-rightArrow" :style="current == calendar.length-1 || {color: color}" @click="current == calendar.length-1 || current++">&#xe6ab;</view>
		</view>
		...
		<swiper class="calendar-date-swiper" :current="current" @change="current = $event.detail.current">
			<swiper-item class="calendar-date-wrapper" v-for="(item,index) in calendar" :key="index">
				<view class="calendar-date" v-for="(value,key) in item" :key="key" :style="value.selected && {background:color,color:'#fff'}">{{value.date}}</view>
			</swiper-item>
		</swiper>
	</view>
</template>
