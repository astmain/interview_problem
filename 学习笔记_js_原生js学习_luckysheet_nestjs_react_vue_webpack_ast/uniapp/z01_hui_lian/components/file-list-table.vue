<template>
	<view>
		<view class="grace-add-list" style="width:100%;">
			<!-- 			<view class="items" v-for="(item, index) in imgLists" :key="index">
				<image v-if="item.fjsfyl==2" :src="item.url" mode="widthFix" @tap="previewImage(item)"></image>
				<image v-else src="http://qiniu.huiliantech.com/word-2.png" mode="widthFix" @tap="previewImage(item)"></image>
				<view class="grace-filename" style="background-color:transparent;;color:#000;font-weight:bold;" 
					v-if="item.fjsfyl!=2" @tap="previewImage(item)">{{item.fname}}</view>
			...
			<view class="grace-news-list" style="width: 100%;">
				<view v-for="(item,index) in imgLists" :key="index" class="garace-hl-list" style="padding: 8px;">
					<view class="grace-news-list-items">
						<view class="grace-news-list-info line-overflow" style="display: flex;"
							@tap="showCollectionItems(item)">
							<text style="font-size: 16px;" class="line-overflow">
								<text class="icon-size iconfont"></text>{{index + 1}}. {{item.fname || ''}}
							</text>
						</view>
						<view class='grace-hl-list-arrow'
							style="margin-top:-36px; padding:0 0px; height: inherit; overflow: inherit;">
							<button @tap="removeImg" v-show="opType && item.candel" :id="'grace-items-img-'+index"
								:data-id="item.id"
								style="height: 28px; width: 52px; line-height: 28px; font-size: 12px;">删除</button>
						</view>
					</view>
					<!-- 					<view class="grace-news-list-info line-overflow" @tap="previewImage(item)">
						<view>
							<text class="icon-size iconfont"></text>上传时间: {{item.cjsj || ''}}
						</view>
					</view> -->

				</view>
			</view>
			<view class="add-btn-text" style="z-index: 100;" @tap="addImg" v-if="btnImg && opType">点击添加附件</view>
			<!-- 			<view class="items add-btn" v-if="btnImg && opType">
		...
		methods: {
			...
			previewImage2: function(item) { //预览图片
				...
				if (item.fjsfyl == 2) //如果是文件的话 
				{
					...
					var supportFile = ".doc,.docx,.xls,.xlsx,.pdf,.ppt,.pptx,";
					if (supportFile.indexOf(item.kzm000) == -1) {
						uni.showModal({
							title:'抱歉，微信小程序暂不支持此类文件查看，请使用app或者web查看文件',
							showCancel:false
						})
						return;
					}
