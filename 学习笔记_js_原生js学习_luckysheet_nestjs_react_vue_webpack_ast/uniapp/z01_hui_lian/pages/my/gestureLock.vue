<template>
	<view style="height: 100vh;overflow: hidden;">
		...
		<view class="lock-tips">
			...
			<view class="small-tips" v-if="lockType == 2 || (lockType == 3 && !needAuthenticate)">请牢记您的手势，忘记后将无法找回</view>
		</view>
		...
		<view class="container-header">
			<view class="lock-reset" @click="back" v-if="lockType==2">返回</view>
			<view class="lock-reset" @click="login" v-if="lockType==1">切换为密码登录</view>
		</view>
		
		<view class="container-confirm" v-if="showLockConfirm">
			<view class="lock-reset" @click="resetLock">重绘</view>
			<view :class="[confirmEnable?'able-confirm':'disable-confirm']" @click="lockConfirm">确认</view>
		</view>
...
<script>
	...
	import mpvueGestureLock from '@/components/gesturePassword/mpvueGestureLock/gestureLocker.vue';
	...
	export default {
		...
		onLoad(options) {
			...
			if(uni.getStorageSync('gesture-pwd')){
				this.serverPwd = uni.getStorageSync('gesture-pwd');
			}
					...
					uni.setStorage({
						...
						success: () => {
							...
							uni.setStorage({
								...
								success: () => {
									this.EventBus.fire('gestureMode',{});
								},
								fail: (res) => {
