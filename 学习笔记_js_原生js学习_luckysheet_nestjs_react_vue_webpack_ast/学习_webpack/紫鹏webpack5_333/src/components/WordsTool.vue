...
<vxe-modal  className="wordstool modifytool" :showHeader="false" @hide="clearWordsTool" v-model="openTool" width="870px" height="550px"
      ...
      <div class="toolcontainer">
        ...
        <div class="closeSec"><div class="close" @click="openTool = false"><i class="xzzicon-close" ></i></div></div>
      </div>
    </vxe-modal>
...
<script setup>
  ...
  const options = reactive([{
   ...
   const   copyAll = () => {
      ...
      navigator.clipboard.writeText(arrToStr);
      ElMessage({
          message: '操作成功',
          type: 'success',
          offset: 70,
    ...
    console.log('e: ', e);
      switch(e){
        case '':      finallyArr.value = [];
                      arrToStr.value = ''
          break;
        case 'one':   arrToStr.value = arr1.join(' '); 
                      finallyArr.value = spArr(arr1);
                      excelArr.value = arr1
          break;
        case 'two':   arrToStr.value = arr2.value.join(' '); 
                      finallyArr.value = spArr(arr2);
                      excelArr.value = arr2.value
          break;
        case 'three': arrToStr.value = arr3.value.join(' '); 
                      finallyArr.value = spArr(arr3);
                      excelArr.value = arr3.value
          break;
        case 'four':  arrToStr.value = arr4.value.join(' ');
                      finallyArr.value = spArr(arr4);
                      excelArr.value = arr4.value
      }
      //  console.log('---------finallyArr:------ ', finallyArr.value);
      ...
      // 得到剔除关键词的初始arr
        ...
        //得到------二词--------
         for(let l = 1; l< ll; l++){
          ...
          //  arr3.push(key + ' ' + lastArr[2] + ' ' + lastArr[1] + ' ' + lastArr[0])
          // arr3 = spArr(arr3)
